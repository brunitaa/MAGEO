...
function MaterialApoyo() {
  ...
  const [ÍtemsMaterialApoyo, setÍtemsMaterialApoyo] = useState([
    { ÍtemName: "", quantity: 0, observations: "" },
  ]);
  const [ÍtemsOtherMaterialApoyo, setÍtemsOtherMaterialApoyo] = useState([
    { ÍtemName: "", quantity: 0, observations: "" },
  ]);
  const [isOpenMaterialApoyo, setIsOpenMaterialApoyo] = useState(false);
  const [isOpenOtherMaterialApoyo, setIsOpenOtherMaterialApoyo] =
    useState(false);
  ...
  const handleAddRow = (type) => {
    if (type === "materialApoyo") {
      setÍtemsMaterialApoyo([
        ...ÍtemsMaterialApoyo,
        { ÍtemName: "", quantity: 0, observations: "" },
      ]);
    } else if (type === "otherMaterialApoyo") {
      setÍtemsOtherMaterialApoyo([
        ...ÍtemsOtherMaterialApoyo,
        { ÍtemName: "", quantity: 0, observations: "" },
      ]);
    }
  };
    ...
    <div className="flex">
      <div>
        <h3 className="text-2xl font-bold mb-4" style={{ margin: "10px 20px" }}>
          2.3.1 Material de Apoyo (Propios)
        </h3>
        ...
        <h3 className="text-2xl font-bold mb-4" style={{ margin: "10px 20px" }}>
          2.3.2 Otros Materiales de Apoyo (Propios)
        </h3>
        <div className="bg-white flex Ítems-center justify-between mt-4">
          <table className="table-auto">
            <thead>
                ...
                <tr key={index}>
                  <td className="border px-4 py-2">
                    <input
                      ...
                      value={Ítem.ÍtemName}
                      onChange={(e) =>
                        handleChange("otherMaterialApoyo", index, e)
                      }
                    />
                  </td>
                  <td className="border px-4 py-2">
                  ...
                  <td className="border px-4 py-2">
                    <Button
                      ...
                      className="bg-red-800 hover:bg-red-700 text-white font-bold py-1 px-2 rounded focus:outline-none focus:shadow-outline"
                      onClick={() =>
                        handleRemoveRow("otherMaterialApoyo", index)
                      }
                    >
                      Eliminar
                    </Button>
